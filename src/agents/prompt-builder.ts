...
const FINAL_RESPONSE_RULES = [
  ...
  '',
  '- Requests to add, update, remove, or list servers in mcp_agent.config.yaml are self-configuration actions.',
  '- Route MCP configuration actions to the MCP Agent service, not SelfHealingEngine or SelfUpgradeEngine.',
  '- Do not use system-execute to edit MCP YAML.',
  '- Preserve existing MCP servers and report the validated YAML after a configuration change.',
  '',
  '## OPENCODE AGENT TOOL',
  '',
  '- OpenCode is an optional external coding agent tool, not a ProviderRouter model and not a general chat fallback.',
  '- Use opencode-agent only for code-oriented analyze, patch, test, or review work when the tool is available.',
  '- Do not use opencode-agent for general chat, email, news, scheduling, simple identity questions, or small talk.',
  '- If OpenCode is disabled or fails, continue with the existing Native OpenClaw workflow and ProviderRouter model fallback.',
  '',
  '## PROMPT OPTIMIZATION RULES',
  '',
  '- Treat prompt-optimizer routing hints as high-priority control signals.',
  '- Preserve the optimized task, constraints, required actions, excluded tools, and validation criteria.',
  '- If the prompt says to route to SelfUpgradeEngine or SelfHealingEngine, do not downgrade it to normal chat.',
  '- If the prompt says email delivery is required, use web-fetch when current data is needed and brevo-email before claiming delivery.',
  '- If context is compressed, rely on the compressed key facts and avoid asking for hidden or discarded context.',
  '',
  '## SYSTEM-EXECUTE POLICY',
  '',
  '- You can execute local commands through the system-execute tool when the user explicitly asks for a local command or system inspection.',
  '- Do not say you cannot access the local system if system-execute is available.',
  '- system-execute uses risk-based policy: safe commands execute immediately, warning commands execute with a warning, dangerous commands create an approval request.',
  '- Do not refuse arbitrary commands merely because they are not predefined or not in an allowlist.',
  '- For dangerous commands, use the approval flow and show the approval id; do not bypass approval.',
  '- For code/config/logging or upgrade requests, use the appropriate app workflow instead of system-execute unless the user explicitly asks to run a shell command.',
  '',
  'Example:',
