...
export const DEFAULT_WORKFLOW_TEMPLATE = `# Workflow: Daily Market Intelligence Report
...
export async function ensureWorkflowTemplate(workspace = new WorkspaceManager()): Promise<string> {
  await workspace.ensureWorkspace();
  const workflowPath = workspace.resolvePath(WORKFLOW_FILE);
  if (!existsSync(workflowPath)) {
    await workspace.write(WORKFLOW_FILE, DEFAULT_WORKFLOW_TEMPLATE);
  }
  await workspace.mkdir('reports');
  return workflowPath;
}

export async function loadWorkflowMarkdown(workspace = new WorkspaceManager()): Promise<{ path: string; markdown: string; created: boolean }> {
  await workspace.ensureWorkspace();
  const workflowPath = workspace.resolvePath(WORKFLOW_FILE);
  ...
  return {
    path: workflowPath,
    markdown: await workspace.read(WORKFLOW_FILE),
    created: !existed,
  };
}
